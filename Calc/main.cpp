...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(
			...
			GM_ADVANCED, 0, 600,	//Escapement, Orientation, Weight
	...
	case WM_CONTEXTMENU:
	{
		...
		HBRUSH hhbrBackground = CreateSolidBrush(RGB(0,0,240));
		switch (TrackPopupMenuEx(hMenu, TPM_BOTTOMALIGN | TPM_LEFTALIGN | TPM_RETURNCMD, LOWORD(lParam), HIWORD(lParam), hwnd, NULL))
		{
		case IDC_SQUARE_BLUE:strcpy(sz_skin, "square_blue");	hhbrBackground =CreateSolidBrush( RGB(0, 0, 240));		break;
		case IDC_SQUARE_GREEN:strcpy(sz_skin, "square_green");	hhbrBackground = CreateSolidBrush (RGB(0, 240, 0));		break;
		case IDC_ROUND_GREEN:strcpy(sz_skin, "round_green");	hhbrBackground = CreateSolidBrush (RGB(240, 0, 0));		break;
		case IDC_EXIT:SendMessage(hwnd, WM_CLOSE, 0, 0);
		}
		SetSkin(hwnd, sz_skin);
		SetClassLongPtr(hwnd, GCLP_HBRBACKGROUND, (LONG_PTR)hhbrBackground);
	}
	break;
